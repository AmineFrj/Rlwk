...
def get_top_features(vocabulary, weights, n_features=20):
    ...
    weigh_word_dict = {term: weight for term, weight in sorted(zip(vocabulary, weights), key=lambda x: x[1], reverse=False)}
...
def get_top_terms_weight(mat, labels, weights, vocabulary, max_count=500, n_features=10):
    ...
    for cluster, top_words in enumerate(top_terms_count):
        term_weights = {vocabulary[word]: round(weights[word], 2) for word in top_words[:max_count]}
        sorted_terms = sorted(term_weights.items(), key=lambda x: x[1])[:n_features]
